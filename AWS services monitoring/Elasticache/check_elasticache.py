...
def get_cluster_info(region, identifier=None):
    ...
    elasticache = boto3.client('elasticache', region_name=region, config=my_config)
    try:
        if identifier:
            ...
            info = [v['CacheClusterId'] for v in elasticache.describe_cache_clusters()['CacheClusters']]
...
def get_cluster_stats(region, node, step, start_time, end_time, metric, identifier):
    ...
    cw = boto3.client('cloudwatch', region_name=region, config=my_config)
    result = cw.get_metric_statistics(
        ...
        Statistics=['Average'],
        Dimensions=[
            {'Name': 'CacheClusterId', 'Value': identifier},
            {'Name': 'CacheNodeId', 'Value': '%04d' % node},
        ]
    )
    if result['Datapoints']:
        if len(result['Datapoints']) > 1:
            # Get the last point
            result['Datapoints'] = sorted(result['Datapoints'], key=lambda x: x['Timestamp'], reverse=True)
